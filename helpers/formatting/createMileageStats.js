...
export async function createMileageStats(motTests, manufactureYearRaw) {
    ...
    const currentMileageDate = format(startOfDay(new Date(newest.date)), 'dd/MM/yyyy');

    let summary = ` - Last known: ${currentMileageString} (${currentMileageDate})\n`;
    ...
    if (validTests.length > 1) {
        ...
        if (maxYearly && maxYearly.amount > 0) {
            summary += ` - Most in a year: ~${formatNum(Math.round(maxYearly.amount / 100) * 100)} mi (${maxYearly.year})\n`;
        }
        if (minYearly && minYearly.amount >= 0 && minYearly.amount !== maxYearly?.amount) {
            summary += ` - Least in a year: ~${formatNum(Math.round(minYearly.amount / 100) * 100)} mi (${minYearly.year})\n`;
        }
    }
    ...
    if (anomaly) {
        summary += ` - ⚠️ Warning: Mileage appears to have been tampered. Mileage decreased from ${formatNum(anomaly.from.value)} ${anomaly.from.unit} (${anomaly.from.date.getFullYear()}) to ${formatNum(anomaly.to.value)} ${anomaly.to.unit} (${anomaly.to.date.getFullYear()})`;
    }
    ...
    const chartConfig = {
        ...
        options: {
            ...
            title: { display: true, text: 'Mileage History', fontColor: 'rgb(255, 255, 255)', fontSize: 16 },
            scales: {
    ...
    let mileageGraphUrl = `https://quickchart.io/chart?bkg=rgb(36,36,41)&w=600&h=300&c=${encodeURIComponent(JSON.stringify(chartConfig))}`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.success && data.url) {
                mileageGraphUrl = data.url;
            }
        }
    } catch (e) {
        // Fallback to the constructed URL if it fails.
        logger.error('QuickChart API error', {
            error: e,
            chartConfig
        });
    }
